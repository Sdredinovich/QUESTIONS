...
export class ThemesService {
  ...
  BROWSER: QuestsArr = [
    ...
    },
    {
      title: 'Cockie',
      quest: `
      Cockie - это данные которые сохраняются и передаются при запросах между браузером и сервером.

      Cockie В основном нужны для бекенда, но клиент так же может получить к ним доступ с помощью document.cookie

      Cockie имеет срок жизни, после которого они удаляются

      Cockie ассоцииируются с доменом, которым они принадлежат. Т.е если они были созданы на сайте example.com,
       только на этом сайте они удут доступны.

      Cockie могут храниться только как строки

      Если проставлен флаг secure, то он будет доступен только через HTTPS
      Если проставлен флаг httponly, то клиент не сможет изменять его.
...
==================================================================================
    ...
    },
    {
      ...
      В JavaScript прокси (Proxy) — это объект, который позволяет перехватывать и изменять базовые операции над другим объектом, такие как чтение, запись, перечисление свойств и т. д.
...
const handler = {
  ...
  getOwnPropertyDescriptor(target, prop) {
    console.log("[GET_DESCRIPTOR] " + prop);
  ...
  setPrototypeOf(target, proto) {
    console.log("[SET_PROTOTYPE] " + proto);
...
Object.getOwnPropertyDescriptor(proxy, "name"); // [GET_DESCRIPTOR] name
proxy.greet();                  // [APPLY] (если greet — функция)
      `,
    },
    {
      title: 'Дексрипрторы',
    ...
    Object.defineProperty(
      obj,
      'name', //название добавляемого свойства
      {
    value: 'John',//Устанавливает первоначальное значение
  ...
  TS: QuestsArr = [
    {
      title: 'ENUM в TS, отличие от объектов',
      quest: `
      Enum это перечисление значений. Enum может выступать как в качестве объекта, так и типа, который мы можем где то указать.

      Отличие Enum от объекта в том, что в ходе работы мы в обхект можем что то добавлять, а в енам не можем.
      В объект мы тоже не можем ничего добавлять, если укажем после объясления оъекта 'as const',
      Также, чтобы мы смогли пользоватся объектом как енамом, то есть указывать как тип, мы можнм поспользоваться утилитарным типом ValueOf<obj>


        Так же enum компилятором обрабатывается так, что из enum ts длает определенную функцию.
        Если мы напишем cost enum, то после компиляции никаких следов не о станется в бандле.
        Поэтому константный енам мы не сможем вывести даже в консоль, в отличие от обычного

        Еще. В тайпскрипте типы с одинаковой структурой взаимозаменяемы, потому что TS язык со структурной типизацией
        А enum - нет

        Пример с type

        type FirstType = { value: 'value' };
        type SecondType = { value: 'value' };

        function compare(type: FirstType) {}

        const second: SecondType = { value: 'value' };

        compare(second) // не вызовет ошибки
        ...
        function compare(type: FirstEnum) {}

        compare(SecondEnum.Value) // вызовет ошибку
...
`,
    ...
    },
    {
      ...
      С помощью абстрактоного класса можно создавать легковесные инжекшен токены(lightweight injection tokens)
      Нельзя расширять, как интерфейс.
...
`,
  ...
  RXJS: QuestsArr = [
    {
      title: 'Отличие Observable от Promise',
      quest: `

      Основные различия:

Одноразовость vs Многоразовость

Promise выполняется единожды — после разрешения (resolve) или отклонения (reject) он завершает свою работу.

Observable может испускать множество значений (поток событий), пока не будет завершён (например, через complete()).

Ленивость (Lazy) vs Энергичность (Eager)

Promise выполняется немедленно при создании, даже если на него ещё не подписались.

Observable не запускается, пока на него не подпишутся (subscribe()).

Отмена выполнения

Promise нельзя отменить после запуска.

Observable поддерживает отмену через unsubscribe().

Множественные подписчики

Promise при каждом вызове .then() получает одно и то же значение.

Observable может работать по-разному для разных подписчиков (если не используется multicast).

Операторы (Функциональный подход)

Promise имеет только .then(), .catch(), .finally().

Observable поддерживает мощные операторы (map, filter, debounceTime, switchMap и т. д.), позволяя удобно трансформировать поток данных.
  ...
  ANGULAR: QuestsArr = [
    {
      title: 'Легковесные инжекшн токены',
      quest: `
      🔹 Инжекшен токен (абстрактный класс) позволяет избежать попадания реализации (сервиса/компонента) в финальный бандл, если она не используется.
...
Поскольку <lib-header> является необязательным, элемент может появиться в шаблоне в своей минимальной форме, <lib-card></lib-card>. В этом случае <lib-header> не используется, и можно было бы ожидать, что он будет в виде дерева, но этого не происходит. Это происходит потому, что LibCardComponent фактически содержит две ссылки на LibHeaderComponent.


@ContentChild(LibHeaderComponent) header: LibHeaderComponent;
Одна из этих ссылок находится в позиции type — то есть, она указывает LibHeaderComponent как тип: header: LibHeaderComponent;.

Другая ссылка находится в позиции value — то есть LibHeaderComponent является значением декоратора параметров @ContentChild(): @ContentChild(LibHeaderComponent).

Компилятор по-разному обрабатывает ссылки на маркеры в этих позициях.

Компилятор стирает ссылки type position после преобразования из TypeScript, поэтому они не влияют на дрожание деревьев.

Компилятор должен сохранять ссылки value position во время выполнения, что предотвращает древовидное дрожание компонента.
...
В этом примере реализация LibCardComponent больше не ссылается на LibHeaderComponent ни в позиции типа, ни в позиции значения. Это позволяет полностью перетряхнуть дерево LibHeaderComponent. Сохраняется LibHeaderToken, но это только объявление класса, без конкретной реализации.
...
`,
    ...
    {
      title: 'ChangeDetection стратегии',
      quest: `
      Причины для вызова механизма обнаружения изменений при стратегии Default:
       - Любые асинхронные операции(setTimeout. Observable, Promise)
       - Срабатывание обработчиков событий
       - Изменение входных данных
       - Срабатывание механизма в содержащем компоненте
       - Срабатывание механизма в содержимом компоненте
       - Изменение сигналов(Срабатывает только в )

      Причины для вызова механизма обнаружения изменений при стратегии OnPush:
       - Изменение входных данных
       - Срабатывание обработчиков событий
       - async pipe (Потому что внутри вызывается markForCheck)
       - Изменение значения сигнала

       this.cdr.detach() - отцепляют ветку дерева компонента от механизма обнаружения изменений
       this.cdr.attach() - цепляет обратно ветку дерева компонента к механизму обнаружения изменений
       this.cdr.markForCheck() - помечает компонент грязным, чтобы при срабатывании в родителе в нем тоже сработало
       this.cdr.detectChanges() - срабатывает механизм обнаружения изменений(только в том компоненте, где он вызывается)

       this.NgZone.runOutsideAngular(fn) - выполняет функцию вне ангуляра
       this.NgZone.run(fn) - выполняет функцию внутри ангуляра

       this.NgZone.hasPendingMicrotasks - возвращает true, если в очереди есть микрозадачи
       this.NgZone.hasPendingMacrotasks - возвращает true, если в очереди есть макрозадачи

      `,
    },
    {
      title: 'Компоненты, Модули',
      quest: `
      Компоненты - это строитеьные блоки, из которых строится ангуляр приложение.
      По сути это директива. Отличие компонента от директивы состоит в том, что у компонента есть шаблон,
      а у директивы нет.

      Модуль - это такой контейнер, который декларирует и инжетит какие то сущности, которые изолирует какой то
      функционал, который может быть переиспользован в другом месте.
...
@Component({
  ...
  ',
  imports: [ProductComponent], // Важно: импорт динамического компонента
})
export class OfficeRegistryComponent implements AfterViewInit {
  @ViewChild('productContainer', { read: ViewContainerRef }) views!: ViewContainerRef;
  ...
  ngAfterViewInit(): void {
    this.products.forEach((item) => {
      const component = this.views.createComponent(ProductComponent);
      component.setInput('product', item); // Передаём данные
    });
  }
}

Или можно добавить в текущий компонент созданный динамический компонент
  ...
  PRINCIPS: QuestsArr = [
    {
      ...
      quest: `
      KISS - Keep It Simple, Stupid - Код должен быть простым
      DRY - Don't Repeat Yourself - Не повторяйся. Легче поддерживать и тестировать
      YAGNI - You Ain't Gonna Need It - Не пиши код, если он еще не нужен`,
    },
    {
      title: 'GRASP',
      quest: `
      1) Информационный эксперт
           Информационный эксперт — это паттерн, который помогает определить, какому классу или объекту следует делегировать ответственность за выполнение задачи.

           Основная идея:

           "Назначайте ответственность тому классу, который обладает максимальной информацией, необходимой для её выполнения."

      2) Creator (Создатель)
            "Класс, который содержит или агрегирует объект, либо активно его использует, должен его и создавать."
            Пример: Если Cart (корзина) содержит CartItem, то Cart и должен создавать CartItem.

      3). Controller (Контроллер)
            Кто обрабатывает системные события?

            "Не заваливайте UI-логикой компоненты, выносите обработку в отдельный контроллер (сервис, хук, стор)."

            Пример: В ангуляре это сервисы, ngrx store, либо же сам компонент, который содержит логику обработки событий

            Зачем?
            Отделяет UI от бизнес-логики, упрощает тестирование.
...
`,
  ...
  PATTERNS: QuestsArr = [
    {
      title: 'forRoot forChild',
      quest: `
Паттерн forRoot и forChild в Angular
Это подход для конфигурации модулей в Angular, особенно полезный для ленивой загрузки (Lazy Loading) и управления провайдерами.

1. Зачем нужен?
forRoot() — настраивает глобальные сервисы/провайдеры (например, RouterModule, StoreModule).

forChild() — регистрирует локальные сервисы/компоненты/директивы для дочерних модулей без повторного создания провайдеров.

2. Основная идея
Избегание дублирования сервисов (например, Router или Store).

Оптимизация производительности (ленивая загрузка модулей).

3. Примеры
a) RouterModule (официальный пример Angular)
typescript
// app.module.ts (корневой модуль)
@NgModule({
  imports: [
    RouterModule.forRoot(routes) // Настройка роутера для всего приложения
  ]
})
export class AppModule {}

// lazy.module.ts (лениво загружаемый модуль)
@NgModule({
  imports: [
    RouterModule.forChild(routes) // Добавление маршрутов без пересоздания сервисов
  ]
})
export class LazyModule {}
...

    {
      ...
      Модели данных:
      DTO - Data Transfer Object - Модель данных бекенда
      VM - View Model - Модель данных вью
      ENTITY - Модель данных внутри в сторе
  ...
  ARCHITECT: QuestsArr = [
    {
      ...
      DDD (Domain-Driven Design) для фронтендера — кратко и понятно
DDD — это подход к разработке, где бизнес-логика (домен) становится центром архитектуры.
Для фронтенда это значит: "Делай код осмысленным, отражай реальные бизнес-процессы, а не просто пиши компоненты".


DDD во фронтенде — это:
🔹 Разделение кода по бизнес-логике (а не по техническим слоям).
🔹 Использование сущностей, value-объектов, сервисов.
🔹 Чёткая структура проекта (папки orders, users, а не components, services).



То есть мы:

🔹 Проектируем архитектуру по доменным областям
🔹 Не раскладываем файлы по типу сущностей(components, pipes, directives)
🔹 Читаем код как описание бизнес-процессов

У нас с бизнесом должен быть общий словарь терминов.
    ...
    },
    {
      ...
      MVP, MVVM, MVC — кратко и понятно
Это архитектурные паттерны, которые помогают организовать код в приложении. Их главная цель — разделить логику, данные и интерфейс, чтобы:

✅ Упростить поддержку кода.

✅ Ускорить разработку (каждый модуль отвечает за своё).

✅ Упростить тестирование (логику можно тестировать без UI).



      Коротко про отличия MVP, MVVM, MVC в Angular
...
/src
  /user
    user.component.ts      # ViewModel (логика + состояние)
    user.component.html    # View (шаблон)
    user.service.ts       # Model (данные)
...
/src
  /user
    user.component.ts      # View (только UI)
    user.presenter.ts     # Presenter (логика)
    user.service.ts       # Model (данные)
...
/src
  /user
    user.controller.ts    # Controller (логика)
    user.model.ts        # Model (данные)
    user.component.html  # View (шаблон)
    `,
    },
  ];
