...
export class ThemesService {
  HRQuestions: QuestsArr = [
    {
      title: 'Какими библиотеками Angular пользуетесь?',
      quest: `

      * Angular, как фреймворк для разработки веб-приложений на языке TypeScript, предоставляет множество библиотек и модулей для облегчения различных задач в процессе разработки. Вот несколько ключевых библиотек и модулей, связанных с Angular:

      * RxJS (Reactive Extensions for JavaScript): Не является прямо Angular-специфичной, но активно используется в Angular. RxJS предоставляет возможности для работы с асинхронными операциями и управления потоками данных.

      * @angular/core: Основная библиотека Angular, которая содержит основные функциональные возможности фреймворка, такие как компоненты, директивы, сервисы, инжекции зависимостей и другие.

      * @angular/forms: Предоставляет инструменты для работы с формами в Angular, включая директивы и сервисы для управления состоянием форм, валидацией и т.д.

      * @angular/http (устаревшая, заменена на HttpClientModule): Предоставляет модуль для работы с HTTP-запросами. Новый модуль HttpClientModule вместо этого предоставляет более современный и удобный способ работать с HTTP в Angular.

      * @angular/router: Позволяет создавать маршруты (routes) в Angular-приложении, обеспечивая навигацию между различными компонентами и представлениями.

      * @angular/material: Библиотека, реализующая компоненты пользовательского интерфейса (UI) в соответствии с принципами Material Design. Это включает в себя кнопки, таблицы, формы и другие элементы интерфейса.

      * NgRx: Предоставляет инструменты для управления состоянием приложения через реактивные принципы. Это особенно полезно для сложных приложений с большим объемом состояния.

      * @angular/animations: Модуль для работы с анимациями в Angular-приложении. Позволяет создавать плавные и интересные анимации для пользовательского интерфейса.

        `,
    },
    {
      title: 'Про текущее место работы',
      quest: `
      Текущее место работы - это образовательная платформа. Точнее платформа для онлайн образования.

      Модули: менеджера задач, личного кабинета, учета времени
      проведенного за решением задач, статьи с комментариями на различные темы.  Есть модуль с материалами для обучения. Так же различные графики успеваемости
      учеников, кто сколько выполнил задач и т.п. Так же включает себя
      модуль с чатами между преподавателями и студентами а так же между студентами
      Естественно имеется админская панель для управления различными параметрами,
      менеджмента участников и прав доступов, разделение на потоки, настройка тарифов, управление различными видами
      материалов для обучение,
      написанием статей, запуском курсов для обучения и тд.

      Как проходит разработка?

      В команде работают 5 программистов, 3 фронтендера и 2 бэкендера. Есть выделенный тимлид.
      Разработка происходит в таск менеджере тайга разработка: Аналитик описывает задачи, тимлид прописывает их в такс менеджере.
      Программисты разбирают задачи, создают свои ветки ведут в них разработку, затем делают пулреквесты в основную ветку.
      Затем задача проходит код-ревью, после возможных доработок задача сливается в главную ветку, уходит на проверку,
      тестировщик проверяет работоспособность приложения, создает новые задачи для фикса.


      Документация ведется в конвлюенсе

      С бэком взаимодействуем с помощью сваггера

      `,
    },
    {
      title: 'Про архитектуру нашего приложения',
      quest: `
  ...
  BROWSER: QuestsArr = [
    ...
    },
    {
      ...
      HTTP (HyperText Transfer Protocol): Это протокол передачи данных, используемый для передачи информации между клиентом и сервером в сети Интернет. Однако данные передаются в незашифрованном виде, что делает их уязвимыми к перехвату и вмешательству.

      HTTPS (HyperText Transfer Protocol Secure): Это защищенная версия HTTP, использующая шифрование для обеспечения конфиденциальности и целостности данных. HTTPS использует протокол TLS/SSL для шифрования данных между клиентом и сервером.
...
`,
    ...
    },
    {
      ...
      quest: `
    AJAX: AJAX (Asynchronous JavaScript and XML) представляет собой технологию, которая позволяет
     обмениваться данными между браузером и сервером в фоновом режиме без перезагрузки всей веб-страницы.
     ПРИМЕР:
...
==================================================================================
    ...
    },
    {
      ...
      quest: `
      Можно сделать глубокое копирование с помощью новой функции structuredClone(obj),
...
`,
  ...
  RXJS: QuestsArr = [
    {
      ...
      switchMap: Принимает функцию, которая возвращает новый Observable. Когда входной Observable выдает новое значение, switchMap отписывается от предыдущего Observable и подписывается на новый. Это полезно, когда вам нужно выполнить новый запрос и отменить предыдущий запрос, если он все еще активен.

      mergeMap (flatMap): Принимает функцию, которая возвращает новый Observable. Каждый раз, когда входной Observable выдает новое значение, mergeMap создает новый Observable и объединяет их в один выходной поток, сохраняя порядок. Этот оператор подходит для выполнения асинхронных операций параллельно.

      concatMap: Принимает функцию, которая возвращает новый Observable. Когда входной Observable выдает новое значение, concatMap создает новый Observable и добавляет его к концу последовательности Observable. Это гарантирует, что операции будут выполнены по порядку.

      exhaustMap: Принимает функцию, которая возвращает новый Observable. Когда входной Observable выдает новое значение, exhaustMap начинает обрабатывать новый Observable только если предыдущий Observable завершился. Если новый Observable получается, пока предыдущий еще не завершен, он игнорируется.
    ...
    },
    {
      ...
      Источник данных будет создан только при налиии подписчиков, без них не будет выдавать значение.
      Каждый новый подписчик - новый источник данных.

      Hot Observable — это поток, источник данных которого создается снаружи конструктора Observable.
      Ему не нужны подписчики, он и так бует выдавать данные. При наличии хоть 10 пдписчиков, источник
      данных будет один. Он будет делиться одними данными со всеми подписчиками.
    ...
    },
    {
     ...
     как только хотя бы один из них излучает новое значение.
  ...
  ANGULAR: QuestsArr = [
    {
      title: 'Данимеские классы в ангуляр',
      quest: `
